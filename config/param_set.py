...
# パラメータyaml作成
def create_param_yaml(paramdate, model, peft, training, flow_keys=("target_modules",)):
    ...
    for m in model_combos:
        for p in peft_combos:
            for t in training_combos:
                ...
                # flow_keys に含まれるキー（例：target_modules）が peft/model/training にあれば
                # その値が list の場合 FlowStyleList でラップする
                for section in ("model", "peft", "training"):
